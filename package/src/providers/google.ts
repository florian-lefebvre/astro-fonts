...
import type { FontProvider, ResolveFontFacesOptions } from "../types.js";
import { extractFontFaceData, addLocalFallbacks } from "../css/parse.js";
...
export const googleProvider = {
	name: "google",
	async setup(_, logger) {
		await initialiseFontMeta(logger);
	},
	async resolveFontFaces(fontFamily, defaults, logger) {
		if (!isGoogleFont(fontFamily)) {
			return;
		}

		return {
			fonts: await cachedData(
				`google:${fontFamily}-${hash(defaults)}-data.json`,
				() => getFontDetails(fontFamily, defaults),
				{
					onError(err) {
						logger.error(
							`Could not fetch metadata for \`${fontFamily}\` from \`google\`. ${err}`,
						);
						return [];
					},
				},
...
async function fetchFontMetadata() {
	return await $fetch<{ familyMetadataList: FontIndexMeta[] }>(
		"https://fonts.google.com/metadata/fonts",
		{ responseType: "json" },
	).then((r) => r.familyMetadataList);
}

async function initialiseFontMeta(logger: AstroIntegrationLogger) {
	fonts = await cachedData("google:meta.json", fetchFontMetadata, {
		onError() {
			logger.error(
				"Could not download `google` font metadata. `@nuxt/fonts` will not be able to inject `@font-face` rules for google.",
			);
			return [];
		},
	});
...
async function getFontDetails(
	family: string,
	variants: ResolveFontFacesOptions,
) {
