...
const main = async () => {
	...
	await run("git push");
	await run("pnpm --filter @astrolicious/fonts build");
	await run("pnpm changeset publish");
	await run("git push --follow-tags");
	const tag = (await run("git describe --abbrev=0")).replace("\n", "");
	await run(
		`gh release create ${tag} --title ${tag} --notes "Please refer to [CHANGELOG.md](https://github.com/astrolicious/fonts/blob/main/package/CHANGELOG.md) for details."`,
	);
};
